...
public class MBridgeNative extends CustomEventNative {
    ...
    protected void loadNativeAd(@NonNull final Context context, @NonNull final CustomEventNativeListener customEventNativeListener, @NonNull final Map<String, Object> localExtras, @NonNull final Map<String, String> serverExtras) {
        ...
        MBridgeAdapterConfiguration.configureMBSDK(appId, appKey, context, new MBridgeSDKManager.MBSDKInitializeListener() {
            ...
            public void onInitializeFailure(String message) {
                failAdapter(CONNECTION_ERROR, "mb SDK init failed: " + message);
            }
        });
    }

    private void loadNative(@NonNull Map<String, String> serverExtras, @NonNull Context context, @NonNull CustomEventNativeListener customEventNativeListener) {
    ...
    public class MBridgeNativeAd extends BaseNativeAd implements NativeListener.NativeAdListener, NativeListener.NativeTrackingListener {
        ...
        MBNativeHandler mNativeHandler;
        ...
        void loadAd() {
            ...
            if (TextUtils.isEmpty(mBid)) {
                mNativeHandler = new MBNativeHandler(properties, mContext);
                mNativeHandler.setAdListener(this);
                mNativeHandler.setTrackingListener(this);
                mNativeHandler.load();
            } else {
                mbBidNativeHandler = new MBBidNativeHandler(properties, mContext);
                mbBidNativeHandler.setAdListener(this);
                mbBidNativeHandler.setTrackingListener(this);
                mbBidNativeHandler.bidLoad(mBid);
            }
        ...
        public void onAdLoaded(List<Campaign> campaigns, int template) {
            if (campaigns == null || campaigns.size() == 0) {
                failAdapter(NETWORK_NO_FILL, "No mb campaign active. Failing " + "adapter.");
        ...
        public void clear(@NonNull View view) {
            ...
            if (mbBidNativeHandler != null) {
                mbBidNativeHandler.unregisterView(view, mCampaign);
            }
        }
        ...
        public void destroy() {
            ...
            MoPubLog.log(getAdNetworkId(), CUSTOM, ADAPTER_NAME, "Finished showing mb " + "native ads. Invalidating adapter...");

            if (mNativeHandler != null) {
                mNativeHandler.release();
                mNativeHandler.setAdListener(null);
                mNativeHandler = null;
            } else if (mbBidNativeHandler != null) {
        ...
        void registerViewForInteraction(View view) {
            if (mNativeHandler != null) {
                ...
                mbBidNativeHandler.registerView(view, mCampaign);
            }
        }
    ...
    private boolean serverDataIsValid(final Map<String, String> serverExtras, Context context) {
        ...
        if (serverExtras != null && !serverExtras.isEmpty()) {
            mAdUnitId = serverExtras.get(MBridgeAdapterConfiguration.UNIT_ID_KEY);
            mPlacementId = serverExtras.get(MBridgeAdapterConfiguration.PLACEMENT_ID_KEY);

            appId = serverExtras.get(MBridgeAdapterConfiguration.APP_ID_KEY);
            appKey = serverExtras.get(MBridgeAdapterConfiguration.APP_KEY);
